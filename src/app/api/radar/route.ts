...
import {
  buildRadarResponse,
  loadRadarDatabaseResult,
  type InternetOutage,
  type RadarResponse,
} from '@/lib/radar/persisted';
 ...
 * RADAR_DATA_MODE selects the live IODA query or the persisted World-State
 * outage capture with the same response contract.
 */
...
async function loadLiveRadar(): Promise<RadarResponse> {
  ...
  if (!res.ok) {
    ...
    return { outages: [], total: 0, timestamp: new Date().toISOString(), source: 'IODA (offline)' };
  }
  ...
  const outages: InternetOutage[] = events
    .filter((e: { location?: string }) => {
      // IODA uses "location" field like "country/RW"
      const code = e.location?.split('/')[1];
      return code && COUNTRY_CENTROIDS[code];
    })
    .map((e: { location: string; score?: number; severity?: string; start?: number; duration?: number; datasource?: string }, i: number) => {
      const code = e.location.split('/')[1];
...
export async function GET() {
  try {
    const snapshot = await loadPersistedSnapshot(loadPersistedRuntimeConfig('RADAR', process.env, 86_400_000), {
      label: 'radar',
      getDatabaseResult: (windowMs) => loadRadarDatabaseResult(windowMs),
      buildDatabaseResponse: buildRadarResponse,
      loadLive: loadLiveRadar,
      warn: (message) => console.warn(message),
    });
    return NextResponse.json(snapshot.response, {
      headers: persistedResponseHeaders('Radar', snapshot, SUCCESS_CACHE_CONTROL),
    });
  } catch (error) {
    if (error instanceof PersistedDatabaseUnavailableError) {
      console.error('[radar] Database mode unavailable:', error.message);
      return NextResponse.json(
        { outages: [], total: 0, error: 'Internet outage database unavailable' },
