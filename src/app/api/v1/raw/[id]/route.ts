...
export async function GET(
  ...
  try {
    ...
    const payload = await new WorldStateService(database).getRawObservationById(id);
    if (payload.rawObservation === null) {
      return NextResponse.json(payload, { status: 404, headers: { 'Cache-Control': 'no-store' } });
    }
    return NextResponse.json(payload, { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
    console.error('[worldstate:v1:raw] Failed to load raw observation:', error instanceof Error ? error.message : error);
    return NextResponse.json({
      rawObservation: null,
      collectionRun: null,
      generatedAt: new Date().toISOString(),
      error: 'Failed to load World-State raw observation',
