...
export const WEATHER_SOURCE_IDS = ['nasa-eonet-weather', 'noaa-nws-alerts'];
...
function mapRow(row: WeatherRow): WeatherEventItem | null {
  ...
  return {
    id: `${isNws ? 'nws' : 'eonet'}-${row.source_event_id}`,
    title: row.title,
    category,
    type: row.event_type ?? 'Event',
    icon: weatherIcon(category),
    ...
    source: row.link ?? (isNws ? 'https://api.weather.gov/alerts/active' : 'NASA EONET'),
    provider: isNws ? 'NOAA/NWS' : 'NASA EONET',
  };
}
