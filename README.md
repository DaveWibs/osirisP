...
|--------|------------|---------|
| **Aviation** | Commercial, Private, Military, Jets, Persisted Military ADS-B | OpenSky Network, airplanes.live, adsb.lol |
...
`/api/v1/operations/summary`, `/api/v1/events`, `/api/v1/events/[id]`,
`/api/v1/runs`, `/api/v1/runs/[id]`, `/api/v1/runs/[id]/raw`,
`/api/v1/raw/[id]` and `/api/v1/markets/quotes`. The existing live dashboard
remains unchanged.
...
The aviation slice supports opt-in `airplanes-live-military` and
`adsb-lol-military` capture for military ADS-B feeds behind `/api/flights`. It
archives raw tar1090-style JSON and normalises durable
`aircraft_position_observations` keyed by ICAO24.
