...
import {
  ...
  type InternetOutageSourceId,
} from '../normalisers/internet-outages.js';
import type {
  BeginRunInput,
  CompleteInternetOutageRunInput,
  CompleteInternetOutageRunResult,
  FailRunInput,
...
export const INTERNET_OUTAGE_PARSER_VERSION = 'ioda-outages-v1';
export const INTERNET_OUTAGE_SCHEMA_VERSION = 1;
export const INTERNET_OUTAGE_LOOKBACK_SECONDS = 86_400;

const TRANSIENT_HTTP_STATUSES = new Set([408, 425, 429, 500, 502, 503, 504]);
const MAX_RETRY_DELAY_MS = 60_000;

export interface InternetOutageCollectionStore {
  beginRun(input: BeginRunInput): Promise<void>;
  completeInternetOutageRun(
...
export interface InternetOutageFetcher {
  fetch(endpoint: string | URL, signal?: AbortSignal): ReturnType<BoundedHttpFetcher['fetch']>;
}

export interface InternetOutageArchiveWriter {
  write(input: ArchiveWriteInput): ReturnType<ArchiveWriter['write']>;
}

export interface InternetOutageCollectorOptions {
  archiveWriter: InternetOutageArchiveWriter;
  clock?: () => Date;
  endpoint: URL;
  fetcher: InternetOutageFetcher;
  logger: Logger;
  maxAttempts: number;
  random?: () => number;
  retryBaseMs: number;
  ...
  store: InternetOutageCollectionStore;
}

export interface InternetOutageCollectionResult extends CompleteInternetOutageRunResult {
  archivePath: string;
  rawHash: string;
  retryCount: number;
}

export class InternetOutageHttpStatusError extends Error {
  constructor(
    ...
    this.name = 'InternetOutageHttpStatusError';
  }
}
...
export function iodaOutageEndpoint(baseEndpoint: URL, timestamp: Date): URL {
  const endpoint = new URL(baseEndpoint.toString());
  const until = Math.floor(timestamp.getTime() / 1000);
  endpoint.searchParams.set('from', String(until - INTERNET_OUTAGE_LOOKBACK_SECONDS));
  endpoint.searchParams.set('until', String(until));
  endpoint.searchParams.set('entityType', 'country');
...
export class InternetOutageCollector {
  readonly sourceId: InternetOutageSourceId;
  ...
  constructor(private readonly options: InternetOutageCollectorOptions) {
  ...
  async collect(signal?: AbortSignal): Promise<InternetOutageCollectionResult> {
    throwIfAborted(signal);
    ...
    for (let attempt = 1; attempt <= this.options.maxAttempts; attempt += 1) {
      try {
        ...
        const retryable =
          failure instanceof InternetOutageHttpStatusError
            ? failure.retryable
        ...
        this.options.logger.warn(
          ...
          'Retrying transient internet-outage collection failure',
        );
  ...
  private async collectAttempt(
    attempt: number,
    signal?: AbortSignal,
  ): Promise<InternetOutageCollectionResult> {
    const runId = this.runIdFactory();
    const startedAt = validDate(this.clock);
    const endpoint = iodaOutageEndpoint(this.options.endpoint, startedAt);
    ...
    try {
      raw = await this.options.fetcher.fetch(endpoint, signal);
      stage = 'archive';
      ...
      if (raw.status < 200 || raw.status > 299) {
        const statusError = new InternetOutageHttpStatusError(
          this.sourceId,
          raw.status,
      ...
      const parsed = normaliseInternetOutageFeed(raw.body, raw.responseReceivedAt);
      const completedAt = validDate(this.clock);
      const completed = await this.options.store.completeInternetOutageRun({
        runId,
        sourceId: this.sourceId,
        parsed,
        ...
        parserVersion: INTERNET_OUTAGE_PARSER_VERSION,
        schemaVersion: INTERNET_OUTAGE_SCHEMA_VERSION,
      ...
      this.options.logger.info(
        ...
        'Internet-outage collection completed',
      );
...
class AttemptError extends Error {
  constructor(readonly cause: unknown) {
    super(cause instanceof Error ? cause.message : 'Internet-outage collection attempt failed');
    this.name = 'AttemptError';
  }
}

export function isInternetOutageSourceId(value: string): value is InternetOutageSourceId {
  return value === GATECH_IODA_OUTAGES_SOURCE_ID;
}
