...
import {
  ...
  type NewsRssSourceId,
} from '../normalisers/news-rss.js';
import type {
  BeginRunInput,
  CompleteNewsRssRunInput,
  CompleteNewsRssRunResult,
  FailRunInput,
...
export const NEWS_RSS_PARSER_VERSION = 'news-rss-v1';
export const NEWS_RSS_SCHEMA_VERSION = 1;

const TRANSIENT_HTTP_STATUSES = new Set([408, 425, 429, 500, 502, 503, 504]);
const MAX_RETRY_DELAY_MS = 60_000;

export interface NewsRssCollectionStore {
  beginRun(input: BeginRunInput): Promise<void>;
  completeNewsRssRun(input: CompleteNewsRssRunInput): Promise<CompleteNewsRssRunResult>;
...
export interface NewsRssFetcher {
  fetch(endpoint: string | URL, signal?: AbortSignal): ReturnType<BoundedHttpFetcher['fetch']>;
}

export interface NewsRssArchiveWriter {
  write(input: ArchiveWriteInput): ReturnType<ArchiveWriter['write']>;
}

export interface NewsRssCollectorOptions {
  archiveWriter: NewsRssArchiveWriter;
  clock?: () => Date;
  endpoint: URL;
  ...
  store: NewsRssCollectionStore;
}

export interface NewsRssCollectionResult extends CompleteNewsRssRunResult {
  archivePath: string;
  rawHash: string;
  retryCount: number;
}

export class NewsRssHttpStatusError extends Error {
  constructor(
    ...
    this.name = 'NewsRssHttpStatusError';
  }
}
...
export class NewsRssCollector {
  readonly sourceId: NewsRssSourceId;
  ...
  constructor(private readonly options: NewsRssCollectorOptions) {
  ...
  async collect(signal?: AbortSignal): Promise<NewsRssCollectionResult> {
    throwIfAborted(signal);
    ...
    for (let attempt = 1; attempt <= this.options.maxAttempts; attempt += 1) {
      try {
        ...
        const retryable =
          failure instanceof NewsRssHttpStatusError
            ? failure.retryable
        ...
        await this.sleep(retryDelayMs(attempt, this.options.retryBaseMs, this.random), signal);
      }
    }
  ...
  private async collectAttempt(
    attempt: number,
    signal?: AbortSignal,
  ): Promise<NewsRssCollectionResult> {
    ...
    try {
      ...
      if (raw.status < 200 || raw.status > 299) {
        const statusError = new NewsRssHttpStatusError(
          this.sourceId,
          raw.status,
      ...
      const parsed = normaliseNewsRssFeed(raw.body, this.sourceId, raw.responseReceivedAt);
      const completedAt = validDate(this.clock);
      const completed = await this.options.store.completeNewsRssRun({
        runId,
        sourceId: this.sourceId,
        parsed,
        ...
        parserVersion: NEWS_RSS_PARSER_VERSION,
        schemaVersion: NEWS_RSS_SCHEMA_VERSION,
...
class AttemptError extends Error {
  constructor(readonly cause: unknown) {
    super(cause instanceof Error ? cause.message : 'News RSS collection attempt failed');
    this.name = 'AttemptError';
  }
}

export function isNewsRssSourceId(value: string): value is NewsRssSourceId {
  return (
    value === BBC_WORLD_RSS_SOURCE_ID ||
    value === ALJAZEERA_ALL_RSS_SOURCE_ID ||
    value === GDACS_NEWS_RSS_SOURCE_ID
  );
}
