...
import {
  ...
  type CryptoPriceSourceId,
} from '../normalisers/crypto-prices.js';
import type {
  BeginRunInput,
  CompleteCryptoPriceRunInput,
  CompleteCryptoPriceRunResult,
  FailRunInput,
...
export const CRYPTO_PRICE_PARSER_VERSION = 'coingecko-simple-price-v1';
export const CRYPTO_PRICE_SCHEMA_VERSION = 1;

const TRANSIENT_HTTP_STATUSES = new Set([408, 425, 429, 500, 502, 503, 504]);
const MAX_RETRY_DELAY_MS = 60_000;

export interface CryptoPriceCollectionStore {
  beginRun(input: BeginRunInput): Promise<void>;
  completeCryptoPriceRun(input: CompleteCryptoPriceRunInput): Promise<CompleteCryptoPriceRunResult>;
...
export interface CryptoPriceFetcher {
  fetch(endpoint: string | URL, signal?: AbortSignal): ReturnType<BoundedHttpFetcher['fetch']>;
}

export interface CryptoPriceArchiveWriter {
  write(input: ArchiveWriteInput): ReturnType<ArchiveWriter['write']>;
}

export interface CryptoPriceCollectorOptions {
  archiveWriter: CryptoPriceArchiveWriter;
  clock?: () => Date;
  endpoint: URL;
  ...
  store: CryptoPriceCollectionStore;
}

export interface CryptoPriceCollectionResult extends CompleteCryptoPriceRunResult {
  archivePath: string;
  rawHash: string;
  retryCount: number;
}

export class CryptoPriceHttpStatusError extends Error {
  constructor(
    ...
    this.name = 'CryptoPriceHttpStatusError';
  }
}
...
export class CryptoPriceCollector {
  readonly sourceId: CryptoPriceSourceId;
  ...
  constructor(private readonly options: CryptoPriceCollectorOptions) {
  ...
  async collect(signal?: AbortSignal): Promise<CryptoPriceCollectionResult> {
    throwIfAborted(signal);
    ...
    for (let attempt = 1; attempt <= this.options.maxAttempts; attempt += 1) {
      try {
        ...
        const retryable =
          failure instanceof CryptoPriceHttpStatusError
            ? failure.retryable
        ...
        this.options.logger.warn(
          ...
          'Retrying transient crypto-price collection failure',
        );
  ...
  private async collectAttempt(
    attempt: number,
    signal?: AbortSignal,
  ): Promise<CryptoPriceCollectionResult> {
    ...
    try {
      ...
      if (raw.status < 200 || raw.status > 299) {
        const statusError = new CryptoPriceHttpStatusError(
          this.sourceId,
          raw.status,
      ...
      const parsed = normaliseCryptoPriceFeed(raw.body, raw.responseReceivedAt);
      const completedAt = validDate(this.clock);
      const completed = await this.options.store.completeCryptoPriceRun({
        runId,
        sourceId: this.sourceId,
        parsed,
        ...
        parserVersion: CRYPTO_PRICE_PARSER_VERSION,
      ...
      this.options.logger.info(
        ...
        'Crypto-price collection completed',
      );
...
class AttemptError extends Error {
  constructor(readonly cause: unknown) {
    super(cause instanceof Error ? cause.message : 'Crypto-price collection attempt failed');
    this.name = 'AttemptError';
  }
}

export function isCryptoPriceSourceId(value: string): value is CryptoPriceSourceId {
  return value === COINGECKO_SIMPLE_PRICE_SOURCE_ID;
}
