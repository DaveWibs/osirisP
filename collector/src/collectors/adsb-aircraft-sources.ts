...
} from '../normalisers/adsb-aircraft.js';
import type {
  BeginRunInput,
  CompleteAdsbAircraftRunInput,
  CompleteAdsbAircraftRunResult,
  FailRunInput,
...
export interface AdsbAircraftCollectionStore {
  beginRun(input: BeginRunInput): Promise<void>;
  completeAdsbAircraftRun(input: CompleteAdsbAircraftRunInput): Promise<CompleteAdsbAircraftRunResult>;
...
export interface AdsbAircraftFetcher {
  fetch(endpoint: string | URL, signal?: AbortSignal): ReturnType<BoundedHttpFetcher['fetch']>;
}

export interface AdsbAircraftArchiveWriter {
  write(input: ArchiveWriteInput): ReturnType<ArchiveWriter['write']>;
}

export interface AdsbAircraftCollectorOptions {
  archiveWriter: AdsbAircraftArchiveWriter;
  clock?: () => Date;
  endpoint: URL;
  fetcher: AdsbAircraftFetcher;
  logger: Logger;
  maxAttempts: number;
  random?: () => number;
  retryBaseMs: number;
  ...
  store: AdsbAircraftCollectionStore;
}

export interface AdsbAircraftCollectionResult extends CompleteAdsbAircraftRunResult {
  archivePath: string;
  rawHash: string;
  retryCount: number;
}

export class AdsbAircraftHttpStatusError extends Error {
  constructor(
    ...
    this.name = 'AdsbAircraftHttpStatusError';
  }
}
...
export class AdsbAircraftCollector {
  readonly sourceId: AdsbAircraftSourceId;
  ...
  constructor(private readonly options: AdsbAircraftCollectorOptions) {
  ...
  async collect(signal?: AbortSignal): Promise<AdsbAircraftCollectionResult> {
    throwIfAborted(signal);
    ...
    for (let attempt = 1; attempt <= this.options.maxAttempts; attempt += 1) {
      try {
        ...
        const retryable =
          failure instanceof AdsbAircraftHttpStatusError
            ? failure.retryable
  ...
  private async collectAttempt(
    attempt: number,
    signal?: AbortSignal,
  ): Promise<AdsbAircraftCollectionResult> {
    ...
    try {
      ...
      if (raw.status < 200 || raw.status > 299) {
        const statusError = new AdsbAircraftHttpStatusError(
          this.sourceId,
          raw.status,
      ...
      const parsed = normaliseAdsbAircraftFeed(raw.body, this.sourceId, raw.responseReceivedAt);
      const completedAt = validDate(this.clock);
      const completed = await this.options.store.completeAdsbAircraftRun({
        runId,
        sourceId: this.sourceId,
        parsed,
        ...
        parserVersion: ADSB_AIRCRAFT_PARSER_VERSION,
        schemaVersion: ADSB_AIRCRAFT_SCHEMA_VERSION,
...
class AttemptError extends Error {
  constructor(readonly cause: unknown) {
    super(cause instanceof Error ? cause.message : 'ADS-B aircraft collection attempt failed');
