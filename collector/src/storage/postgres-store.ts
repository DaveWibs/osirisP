...
import type { NormalisedMarketQuoteFeed } from '../normalisers/market-quotes.js';
...
export interface CompleteAirQualityRunInput {
  runId: string;
  sourceId: string;
  parsed: NormalisedAirQualityFeed;
  responseReceivedAt: Date;
  completedAt: Date;
  feedContentHash: string;
  archivePath: string;
...
export type CompleteAirQualityRunResult = CompleteSpaceWeatherRunResult;

export interface CompleteCryptoPriceRunInput {
  runId: string;
  sourceId: string;
  parsed: NormalisedCryptoPriceFeed;
  responseReceivedAt: Date;
  completedAt: Date;
  feedContentHash: string;
  archivePath: string;
...
export interface CompleteInternetOutageRunInput {
  runId: string;
  sourceId: string;
  parsed: NormalisedInternetOutageFeed;
  responseReceivedAt: Date;
  completedAt: Date;
  feedContentHash: string;
  archivePath: string;
...
export type CompleteInternetOutageRunResult = CompleteSpaceWeatherRunResult;

export interface CompleteMarketQuoteRunInput {
  runId: string;
  sourceId: string;
  parsed: NormalisedMarketQuoteFeed;
  responseReceivedAt: Date;
  completedAt: Date;
  feedContentHash: string;
  archivePath: string;
...
export class PostgresStore {
  ...
  async completeAirQualityRun(
    input: CompleteAirQualityRunInput,
  ): Promise<CompleteAirQualityRunResult> {
    this.validateAirQualityCompletionInput(input);

    const client = await this.pool.connect();
    let result: CompleteAirQualityRunResult | undefined;
    let completionError: unknown;
    let destroyClient = false;
    ...
    try {
      ...
      for (const record of input.parsed.records) {
        assertContentHash(record.contentHash, `record ${record.sourceStationId} contentHash`);
        const existingResult = await client.query<ExistingRawObservation>(
          `SELECT
             id,
             source_updated_at,
             metadata ->> 'measurement_content_hash' AS feature_content_hash,
             schema_version,
             parser_version
           FROM raw_observations
           WHERE source_id = $1
             AND source_record_id = $2
           FOR UPDATE`,
          [input.sourceId, record.sourceStationId],
        );
        const existing = existingResult.rows[0];
        const decision = persistenceDecision(
          existing,
        ...
        const rawObservationId = await this.upsertAirQualityRawObservation(
          client,
          input,
          record,
          decision !== 'unchanged',
        );
        await this.upsertAirQualityObservation(
          client,
          input,
          record,
          rawObservationId,
      ...
      assertOneRow(completion.rowCount, 'Completing an air-quality collection run');
      ...
      try {
        ...
        completionError = new AggregateError(
          [error, rollbackError],
          'Crypto-price completion and transaction rollback both failed',
        );
      }
    } finally {
      client.release(destroyClient);
    ...
    if (completionError !== undefined) {
      try {
        ...
        throw new AggregateError(
          [completionError, failurePersistenceError],
          'Crypto-price completion failed and its failure could not be persisted',
          { cause: failurePersistenceError },
        );
      }
      ...
      throw new Error('Crypto-price completion failed', { cause: completionError });
    }

    if (result === undefined) {
      throw new Error('Crypto-price completion ended without a result');
    }

    return result;
  }
  async completeCryptoPriceRun(
    input: CompleteCryptoPriceRunInput,
  ): Promise<CompleteCryptoPriceRunResult> {
    this.validateCryptoPriceCompletionInput(input);

    const client = await this.pool.connect();
    let result: CompleteCryptoPriceRunResult | undefined;
    let completionError: unknown;
    let destroyClient = false;
    ...
    try {
      ...
      for (const record of input.parsed.records) {
        assertContentHash(record.contentHash, `record ${record.sourceAssetId} contentHash`);
        const existingResult = await client.query<ExistingRawObservation>(
          `SELECT
             id,
             source_updated_at,
             metadata ->> 'price_content_hash' AS feature_content_hash,
             schema_version,
             parser_version
           FROM raw_observations
           WHERE source_id = $1
             AND source_record_id = $2
           FOR UPDATE`,
          [input.sourceId, record.sourceAssetId],
        );
        const existing = existingResult.rows[0];
        const decision = persistenceDecision(
          existing,
        ...
        const rawObservationId = await this.upsertCryptoPriceRawObservation(
          client,
          input,
          record,
          decision !== 'unchanged',
        );
        await this.upsertCryptoPriceObservation(
          client,
          input,
          record,
          rawObservationId,
      ...
      assertOneRow(completion.rowCount, 'Completing a crypto-price collection run');
      ...
      try {
        ...
        completionError = new AggregateError(
          [error, rollbackError],
          'Air-quality completion and transaction rollback both failed',
        );
      }
    } finally {
      client.release(destroyClient);
    ...
    if (completionError !== undefined) {
      try {
        ...
        throw new AggregateError(
          [completionError, failurePersistenceError],
          'Air-quality completion failed and its failure could not be persisted',
          { cause: failurePersistenceError },
        );
      }
      ...
      throw new Error('Air-quality completion failed', { cause: completionError });
    }

    if (result === undefined) {
      throw new Error('Air-quality completion ended without a result');
    }
  ...
  async completeInternetOutageRun(
    input: CompleteInternetOutageRunInput,
  ): Promise<CompleteInternetOutageRunResult> {
    this.validateInternetOutageCompletionInput(input);

    const client = await this.pool.connect();
    let result: CompleteInternetOutageRunResult | undefined;
    let completionError: unknown;
    let destroyClient = false;
    ...
    try {
      ...
      for (const record of input.parsed.records) {
        ...
        const existingResult = await client.query<ExistingRawObservation>(
          `SELECT
             id,
             source_updated_at,
             metadata ->> 'outage_content_hash' AS feature_content_hash,
             schema_version,
             parser_version
           FROM raw_observations
           WHERE source_id = $1
        ...
        const rawObservationId = await this.upsertInternetOutageRawObservation(
          client,
          input,
          record,
          decision !== 'unchanged',
        );
        await this.upsertInternetOutageObservation(
          client,
          input,
          record,
          rawObservationId,
      ...
      assertOneRow(completion.rowCount, 'Completing an internet-outage collection run');
      ...
      try {
        ...
        completionError = new AggregateError(
          [error, rollbackError],
          'Internet-outage completion and transaction rollback both failed',
        );
      }
    } finally {
      client.release(destroyClient);
    ...
    if (completionError !== undefined) {
      try {
        ...
        throw new AggregateError(
          [completionError, failurePersistenceError],
          'Internet-outage completion failed and its failure could not be persisted',
          { cause: failurePersistenceError },
        );
      }

      if (completionError instanceof Error) throw completionError;
      throw new Error('Internet-outage completion failed', { cause: completionError });
    }

    if (result === undefined) {
      throw new Error('Internet-outage completion ended without a result');
    }
    return result;
  }

  async completeMarketQuoteRun(input: CompleteMarketQuoteRunInput): Promise<CompleteMarketQuoteRunResult> {
    this.validateMarketQuoteCompletionInput(input);

    const client = await this.pool.connect();
    let result: CompleteMarketQuoteRunResult | undefined;
    let completionError: unknown;
    let destroyClient = false;
    ...
    try {
      ...
      for (const record of input.parsed.records) {
        assertContentHash(record.contentHash, `record ${record.sourceQuoteId} contentHash`);
        const existingResult = await client.query<ExistingRawObservation>(
          `SELECT
             id,
             source_updated_at,
             metadata ->> 'quote_content_hash' AS feature_content_hash,
             schema_version,
             parser_version
           FROM raw_observations
           WHERE source_id = $1
             AND source_record_id = $2
           FOR UPDATE`,
          [input.sourceId, record.sourceQuoteId],
        );
        const existing = existingResult.rows[0];
        const decision = persistenceDecision(
          existing,
        ...
        const rawObservationId = await this.upsertMarketQuoteRawObservation(
          client,
          input,
          record,
          decision !== 'unchanged',
        );
        await this.upsertMarketQuoteObservation(
          client,
          input,
          record,
          rawObservationId,
      ...
      assertOneRow(completion.rowCount, 'Completing a market quote collection run');
      ...
      try {
        ...
        completionError = new AggregateError(
          [error, rollbackError],
          'Market quote completion and transaction rollback both failed',
        );
      }
    } finally {
      client.release(destroyClient);
    ...
    if (completionError !== undefined) {
      try {
        ...
        throw new AggregateError(
          [completionError, failurePersistenceError],
          'Market quote completion failed and its failure could not be persisted',
          { cause: failurePersistenceError },
        );
      }

      if (completionError instanceof Error) throw completionError;
      throw new Error('Market quote completion failed', { cause: completionError });
    }

    if (result === undefined) throw new Error('Market quote completion ended without a result');
    return result;
  }
  ...
  private validateAirQualityCompletionInput(input: CompleteAirQualityRunInput): void {
    assertNonEmpty(input.runId, 'runId');
    assertNonEmpty(input.sourceId, 'sourceId');
    assertValidDate(input.responseReceivedAt, 'responseReceivedAt');
    assertValidDate(input.completedAt, 'completedAt');
    ...
    if (input.parsed.sourceId !== input.sourceId) {
      throw new Error('Parsed air-quality feed source ID does not match the collection run source ID');
    }
  ...
  private validateCryptoPriceCompletionInput(input: CompleteCryptoPriceRunInput): void {
    assertNonEmpty(input.runId, 'runId');
    assertNonEmpty(input.sourceId, 'sourceId');
    assertValidDate(input.responseReceivedAt, 'responseReceivedAt');
    assertValidDate(input.completedAt, 'completedAt');
  ...
  private validateInternetOutageCompletionInput(input: CompleteInternetOutageRunInput): void {
    assertNonEmpty(input.runId, 'runId');
    assertNonEmpty(input.sourceId, 'sourceId');
    assertValidDate(input.responseReceivedAt, 'responseReceivedAt');
    assertValidDate(input.completedAt, 'completedAt');
    ...
    if (input.parsed.sourceId !== input.sourceId) {
      throw new Error('Parsed internet-outage feed source ID does not match the collection run source ID');
    }
  ...
  private validateMarketQuoteCompletionInput(input: CompleteMarketQuoteRunInput): void {
    assertNonEmpty(input.runId, 'runId');
    assertNonEmpty(input.sourceId, 'sourceId');
    assertValidDate(input.responseReceivedAt, 'responseReceivedAt');
    assertValidDate(input.completedAt, 'completedAt');
  ...
  private async upsertAirQualityRawObservation(
    client: PoolClient,
    input: CompleteAirQualityRunInput,
    record: NormalisedAirQualityFeed['records'][number],
    updateSnapshot: boolean,
  ): Promise<string> {
    const candidateId = randomUUID();
    const result = await client.query<{ id: string }>(
    ...
    const row = result.rows[0];
    if (row === undefined) {
      throw new Error(`Raw observation upsert returned no ID for ${record.sourceStationId}`);
    }
    return row.id;
  }
  ...
  private async upsertAirQualityObservation(
    ...
    await client.query(
      `INSERT INTO air_quality_observations AS current (
         id,
         source_id,
         source_station_id,
         observed_at,
       ...
       ON CONFLICT ON CONSTRAINT air_quality_observations_source_station_unique
       DO UPDATE SET
         observed_at = EXCLUDED.observed_at,
         updated_at = EXCLUDED.updated_at,
  ...
  private async upsertCryptoPriceRawObservation(
    client: PoolClient,
    input: CompleteCryptoPriceRunInput,
    ...
    const row = result.rows[0];
    if (row === undefined) {
      throw new Error(`Raw observation upsert returned no ID for ${record.sourceAssetId}`);
    }
    return row.id;
  }

  private async upsertCryptoPriceObservation(
    client: PoolClient,
    input: CompleteCryptoPriceRunInput,
    record: NormalisedCryptoPriceFeed['records'][number],
    rawObservationId: string,
    ...
    await client.query(
      `INSERT INTO crypto_price_observations AS current (
         id,
         source_id,
         source_asset_id,
         observed_at,
       ...
       ON CONFLICT ON CONSTRAINT crypto_price_observations_source_asset_unique
       DO UPDATE SET
         observed_at = EXCLUDED.observed_at,
         updated_at = EXCLUDED.updated_at,
  ...
  private async upsertInternetOutageRawObservation(
    client: PoolClient,
    input: CompleteInternetOutageRunInput,
    record: NormalisedInternetOutageFeed['records'][number],
    updateSnapshot: boolean,
  ): Promise<string> {
    const candidateId = randomUUID();
    const result = await client.query<{ id: string }>(
  ...
  private async upsertInternetOutageObservation(
    client: PoolClient,
    input: CompleteInternetOutageRunInput,
    record: NormalisedInternetOutageFeed['records'][number],
    rawObservationId: string,
    ...
    await client.query(
      `INSERT INTO internet_outage_observations AS current (
         id,
         source_id,
         source_event_id,
         observed_at,
       ...
       ON CONFLICT ON CONSTRAINT internet_outage_observations_source_event_unique
       DO UPDATE SET
         observed_at = EXCLUDED.observed_at,
         updated_at = EXCLUDED.updated_at,
  ...
  private async upsertMarketQuoteRawObservation(
    client: PoolClient,
    ...
    const row = result.rows[0];
    if (row === undefined) {
      throw new Error(`Raw observation upsert returned no ID for ${record.sourceQuoteId}`);
    }
    return row.id;
  }

  private async upsertMarketQuoteObservation(
    client: PoolClient,
    input: CompleteMarketQuoteRunInput,
    record: NormalisedMarketQuoteFeed['records'][number],
    rawObservationId: string,
    ...
    await client.query(
      `INSERT INTO market_quote_observations AS current (
         id,
         source_id,
         source_quote_id,
         observed_at,
       ...
       ON CONFLICT ON CONSTRAINT market_quote_observations_source_quote_unique
       DO UPDATE SET
         observed_at = EXCLUDED.observed_at,
         updated_at = EXCLUDED.updated_at,
         symbol = EXCLUDED.symbol,
